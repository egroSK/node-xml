...
XMLP.prototype._parse = function() {
    ...
    if (fc !== '<' && fc !== '&') {
        ...
        return this._parseCDATA  (this.m_iP + 9);
			...
			var end = this.m_xml.indexOf(">", this.m_iP);
			var next_start = this.m_xml.indexOf("<", this.m_iP + 1);
			if ((next_start > -1) && (next_start < end)) {
				this.m_iP++;
				return this._parse();
			}
        ...
        var position = ++this.m_iP;
        var end = this.m_xml.indexOf(";", position);
        if (end === -1) {
            return this._parse();
        }
        return this._parseEntity(position);
    }
    else{
        return this._parseText   (this.m_iP);
    }
...
XMLP.prototype._parseAttribute = function(iB, iE) {
    ...
    iVB = SAXStrings.indexOfNonWhitespace(this.m_xml, iEq, iE) + 1;
   if (iVB > iE) {
	    return this._setErr(XMLP.ERR_ATT_VALUES);
    }
    ...
    cQuote = this.m_xml.charAt(iVB);
		...
		if (quotes) {
			...
			iVE = SAXStrings.indexOfWhitespace(this.m_xml, iB + 1);
		}
    ...
    strV = (quotes) ? this.m_xml.substring(iVB + 1, iVE) : this.m_xml.substring(iVB, iVE + 1);
...
XMLP.prototype._parsePI = function(iB) {
    ...
    if(iE   == -1) {
        ...
        var end_position = this.m_xml.indexOf(">", iB);
        var next_start_position = this.m_xml.indexOf("<", iB);

        if (next_start_position < end_position) {
            this.m_iP = next_start_position;
            return XMLP._UNCLOSED;
        }
...
SaxParser.prototype._parseLoop = function(parser) {
    ...
    while(!this.m_bErr) {
        ...
        else if(iEvent == XMLP._UNCLOSED) {
            // unclosed comment, instruction, ...; ignore (start) chars and continue parsing
        }
